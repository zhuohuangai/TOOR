...
class MT(nn.Module):
    ...
    def forward(self, x, y, model, mask):
        ...
        y, _ = model(x) # recompute y since y as input of forward function is detached
        return (F.mse_loss(y.softmax(1), y_hat.softmax(1).detach(), reduction="none").mean(1) * mask).mean() #, y_hat
