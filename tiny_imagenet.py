...
def loadImage(path):
    # resize TinyImageNet images to 32x32
    img = misc.imread(path, mode='RGB')
    img = misc.imresize(img, (32, 32, 3))
    return img # Image.fromarray(trans_img.swapaxes(0, 2).swapaxes(1, 2).astype('uint8'), 'RGB')
...
def make_dataset(root, base_folder, dirname, class_to_idx):   
    ...
    if dirname == 'train':
        ...
        with open(imgs_annotations) as r:
            data_info = list(map(lambda s : s.split('\t'), r.readlines()))
        ...
        for imgname in sorted(os.listdir(imgs_path)):
            if is_image_file(imgname):
                ...
                images.append(loadImage(path))
                label = class_to_idx[cls_map[imgname]]
                target.append(label)
...
class TinyImageNet200(data.Dataset):
    ...
    def __init__(self, root, train=True,
        ...
        self.data, self.target = make_dataset(self.root, self.base_folder, dirname, class_to_idx)
